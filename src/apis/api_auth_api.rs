...
pub async fn login(
...
) -> Result<crate::models::AccessToken, Error<LoginError>> {
    ...
    if let Some(ref local_var_str) = client_secret {
        local_var_req_builder =
            local_var_req_builder.query(&[("client_secret", &local_var_str.to_string())]);
    }
...
pub async fn login_user(
...
) -> Result<crate::models::AccessToken, Error<LoginUserError>> {
    let local_var_client = &configuration.client;
    ...
    if let Some(ref local_var_str) = associative {
        local_var_req_builder =
            local_var_req_builder.query(&[("associative", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_user_agent) = configuration.user_agent {
        local_var_req_builder =
            local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
